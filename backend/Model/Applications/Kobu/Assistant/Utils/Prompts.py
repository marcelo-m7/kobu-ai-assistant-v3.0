...
class Prompts(PromptsAttributes):
    ...
    """
    async def prompt_chooser(self, current_conversation_stage: Stages = Stages.WELCOME_STAGE) -> ChatPromptTemplate:
        match current_conversation_stage:
            case Stages.WELCOME_STAGE:
            ...
            case Stages.ACCEPTANCE_OF_TERMS_STAGE:
                prompt = ChatPromptTemplate.from_messages([
                    self._assistant_tone_of_voice(),
            ...
            case Stages.CHOOSE_SUBJECT_STAGE:
                prompt = ChatPromptTemplate.from_messages([
                    self._assistant_tone_of_voice(),
                    ("system", "Conversation history: {conversation_history}"),
                    ("user", "{input}"),
            ...
            case Stages.DATA_COLLECTING_STAGE:
                prompt = ChatPromptTemplate.from_messages([
            ...
            case Stages.DATA_COLLECTING_VALIDATION_STAGE:
                prompt = ChatPromptTemplate.from_messages([
            ...
            case Stages.FREE_CONVERSATION_STAGE:
                prompt = ChatPromptTemplate.from_messages([
                    self._assistant_site_context(),
                    ("system", "{basic_instructions}"),
                    self._assistant_tone_of_voice(),
    ...
    def _assistant_site_context(self) -> tuple:
        ...
        print("_assistant_site_context (): ", self.extra_context)
        return _assistant_site_context

    @staticmethod
    def _assistant_tone_of_voice(tone = None) -> tuple:
        """
